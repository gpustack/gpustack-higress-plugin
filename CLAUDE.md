...
- **Rejection body must be valid JSON when the request had `stream:true`**. `gpustack-token-usage` runs at priority 910 (>600), so on a streaming request it decodes the body first, sets `IsStreamingResponse=true`, and on the encode chain pipes every response chunk through `mergeLargeUsageChunks`. That helper does `json.Valid(chunk)` — non-JSON chunks are treated as incomplete SSE fragments, buffered into `IncompleteChunkData`, and returned as `nil`; `processTokenUsage` then replaces the response body with `[]byte("")`. The downstream client sees the rejection headers (`Content-Length: 28`, `x-ratelimit-limited-key`, ...) but zero body bytes. `rejected()` therefore wraps the message in an OpenAI-style `{"error":{"message":...,"type":"rate_limit_exceeded","code":N}}` envelope and sets `content-type: application/json`. If `RejectedMsg` is already a JSON object it is passed through verbatim. Don't switch the body back to plain text.
