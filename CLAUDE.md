...
- **Re-syncing is a wipe-and-replay, not a merge.** Delete the vendored tree, re-extract `git archive <commit> plugins/wasm-go/extensions/ai-proxy`, then replay the divergences in order: drop the four hgctl files, rewrite the module path, set `pluginName`, insert attribution headers immediately before each `package` line, suffix `VERSION`, re-insert the three README fork notices, re-apply the `mergeSystemMessages` patch, rewrite `FORK_BASE`, `gofmt -w`. Verify by normalising the local tree back (strip headers, reverse the path rewrite, `gofmt` **both** sides — upstream ai-proxy is not gofmt-clean, so without this ~20 files show phantom import-order diffs) and diffing against the upstream tree: the only real divergence must be `claude_to_openai.go`, `claude_to_openai_test.go`, the three READMEs, `VERSION`, `go.mod`, `FORK_BASE`, plus header-only diffs in `main.go` and `test/provider_wasm_smoke.go` (their headers sit after a pre-existing leading comment, so a `head -1` normaliser misses them).
