...
> to wasm) plus `scripts/generate_metadata.py`. The following files came along with the
> fork but are no longer consumed here, so they were removed:
>
> - `option.yaml` — `hgctl` plugin project config (build/test/install arguments, config
>   struct reflection entry point); this repository never invokes hgctl.
> - `Makefile` — upstream's tinygo-based single-plugin build script, superseded by
>   `extensions/Makefile`.
> - `envoy.yaml` — upstream's Envoy config for local hgctl testing.
> - `.gitignore` — hgctl's generated allowlist-style ignore rules (they also swallow
>   files such as `VERSION`), conflicting with this repository's root `.gitignore`;
>   removed to align with the other plugins.
>
> The `test/` directory (upstream hgctl e2e test helpers) is kept for now, but neither the
> unit tests nor the build in this repository depend on it.
