...
>
> ```shell
> make -C extensions build PLUGIN_NAME=gpustack-ai-proxy   # 产物落到 gpustack_higress_plugins/plugins/
> make -C extensions test  PLUGIN_NAME=gpustack-ai-proxy
> ```
>
> 下文提到的 `envoy.yaml`、`Makefile`、`option.yaml` 已在 fork 时删除（原因见 README.md 的 Fork 说明），
> 本地起 Envoy 调试时需要自行从上游仓库取一份。以下内容仅作为上游文档留存。
