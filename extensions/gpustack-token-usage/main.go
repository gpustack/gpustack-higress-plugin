...
const (
	...
	// requestIDHeader is Envoy's per-request id, generated by the HCM at the
	// edge before any filter runs. It is the id reported as `request_id` and
	// the one the access log already carries, which is what lets a caller
	// quote a single value and have it resolve in both places.
	requestIDHeader = "x-request-id"

	// defaultRequestIDResponseHeader is where that id is echoed back so the
	// caller has something to quote at all. Envoy does not return x-request-id
	// downstream unless the HCM sets always_set_request_id_in_response (it
	// defaults to false), and this plugin can add the header itself without an
	// EnvoyFilter, so it does.
	//
	// A GPUStack-specific name rather than plain `x-request-id`: some upstreams
	// put an `x-request-id` of their own on the response (ai-proxy reads one),
	// and reusing the name would leave the caller unable to tell whose id they
	// are holding — while the value we report is unambiguously the gateway's.
	// Override via the `requestIDResponseHeader` config field; setting it to
	// `x-request-id` is allowed and simply overwrites whatever was there.
	defaultRequestIDResponseHeader = "X-GPUStack-Request-Id"
...
const (
	...
	// RequestIDKey holds Envoy's x-request-id, captured once at the request
	// header phase. Read there rather than at report time because a local
	// reply can replace the header set by then, and because the request phase
	// is the only point where the value is unambiguously the inbound one.
	RequestIDKey = "gpustack_request_id"

	// UpstreamResponseIDKey holds the id the model's own response carries:
	// `id` for OpenAI Chat Completions and for every non-streaming body,
	// `response.id` for the Responses API, `message.id` for Anthropic's
	// message_start. Captured once and then short-circuited, so the probe
	// costs one gjson lookup on the first chunk and nothing after.
	UpstreamResponseIDKey = "gpustack_upstream_response_id"
...
type PluginConfig struct {
	...
	// RequestIDResponseHeader is the response header carrying the reported
	// request_id back to the caller. Empty switches the echo off; the id is
	// still reported, it just stops being quotable from the client side.
...
type ModelUsageMetrics struct {
	...
	// TTFTMillis is the interval from request entry to the first response body
	// chunk, streaming only; absent on the non-streaming path, which never sees
	// intermediate chunks. Same value the client is handed as
	// `usage.time_to_first_token_ms`, deliberately: two numbers under one name
	// that disagree would be worse than one that is only approximately what its
	// name says. Its baseline is a time.Now() taken in the same request-header
	// call as StartedAt, so `started_at + ttft_ms` lands on the first chunk to
	// within a millisecond.
	//
	// No duration field beside it: StartedAt and CompletedAt are both on the
	// wire and both stored, so a duration here would only be a third number
	// that can disagree with them.
	TTFTMillis *int64 `json:"ttft_ms,omitempty"`
	// RequestID is Envoy's x-request-id. Present on every tracked request
	// regardless of endpoint or outcome, which is what makes it the id to
	// locate a record by -- an interrupted stream or a request that never
	// reached the model still has one, and it is the same value the access log
	// carries.
	//
	// Not a unique key. It identifies a downstream request, and this report is
	// emitted once per filter-chain run: a fallback pass is an internal
	// redirect of the same downstream request, so two reports can carry one
	// value. A consumer must index it, not constrain it.
	RequestID *string `json:"request_id,omitempty"`
	// UpstreamResponseID is the model's own id for this response
	// (`chatcmpl-...`, `resp_...`, `msg_...`, and `embd-...` from a vLLM
	// embeddings call). Whatever the upstream puts there is taken verbatim --
	// see upstreamResponseID, which is deliberately not gated on the endpoint.
	// Absent when the upstream mints none (OpenAI's own embeddings shape),
	// when the endpoint returns no JSON at all (TTS, image), and when no
	// response arrived. Reported beside RequestID rather than instead of it
	// because it is what a caller reading an SDK response actually has in
	// hand, and the only id a third-party provider can be asked about.
...
func parseConfig(json gjson.Result, config *PluginConfig) error {
	...
	// Absent takes the default; an explicit empty string switches the echo
	// off. Distinguishing the two is the point of the Exists() check — an
	// operator who does not want the header on their responses has no other
	// way to say so.
	if h := json.Get("requestIDResponseHeader"); h.Exists() {
		config.RequestIDResponseHeader = h.String()
	} else {
		config.RequestIDResponseHeader = defaultRequestIDResponseHeader
	}
...
func prepareMetrics(ctx wrapper.HttpContext) bool {
	...
	// Envoy generates this in the HCM before any filter runs, so an empty
	// value means the listener has generate_request_id off -- worth a debug
	// line, since it silently costs every report its locating id.
	if id, err := proxywasm.GetHttpRequestHeader(requestIDHeader); err == nil && id != "" {
		ctx.SetContext(RequestIDKey, id)
	} else {
		proxywasm.LogDebugf("prepareMetrics: no %s on the request; reports will carry no request_id", requestIDHeader)
	}
...
func onHttpResponseHeaders(ctx wrapper.HttpContext, config PluginConfig) types.Action {
	...
	// Echoed here rather than at report time, so it also lands on a rejection
	// that never reached an upstream and is therefore never reported -- being
	// able to quote an id for a refused request is worth more than keeping the
	// header and the usage row in exact correspondence. The value is the
	// inbound one captured at request entry, so what the caller reads is what
	// the report carries.
	//
	// Which rejections those are follows from the filter order, and is narrower
	// than it looks. Envoy runs a local reply through the encoder filters
	// *preceding* the one that raised it, and the encoder chain is the reverse
	// of the request chain -- so this phase is reached only for a reply raised
	// by a plugin that sits after token-usage (400) in the request chain, i.e.
	// lower priority: ext-auth's 401/403 (360), ai-proxy's errors (100). A
	// rejection from higher up -- gpustack-rate-limit's 429 at 600 -- preempts
	// this plugin's request phase entirely, so the request is never tracked and
	// carries no header.
	echoRequestID(ctx, config)
	if !processBody {
...
// echoRequestID puts the reported request id on the response so a caller has
// something to quote when asking about a request.
//
// Replace rather than Add: an upstream may have set a header of this name
// itself, and two values under one name would leave the caller unable to tell
// which one resolves against the usage record. The reported id is the one that
// does, so it is the one that survives.
func echoRequestID(ctx wrapper.HttpContext, config PluginConfig) {
	if config.RequestIDResponseHeader == "" {
		return
	}
	requestID := ctx.GetStringContext(RequestIDKey, "")
	if requestID == "" {
		return
	}
	if err := proxywasm.ReplaceHttpResponseHeader(config.RequestIDResponseHeader, requestID); err != nil {
		proxywasm.LogDebugf("echoRequestID: failed to set %s: %v", config.RequestIDResponseHeader, err)
	}
}
...
func handleNonStreamingResponseBody(ctx wrapper.HttpContext, data []byte, endOfStream bool, maxBytes int) {
	...
	captureUpstreamResponseID(ctx, body)
...
// captureUpstreamResponseID records the model's own id for this response, the
// first time one appears.
//
// Short-circuiting on the captured value is what keeps this off the hot path:
// the id arrives in the first chunk of a stream, so every chunk after it costs
// one context lookup instead of a JSON probe. Capturing the *first* is also the
// correct choice rather than an optimisation — a stream's chunks all repeat one
// id, so a later one can only be the same value or a malformed frame.
func captureUpstreamResponseID(ctx wrapper.HttpContext, payload []byte) {
	if ctx.GetStringContext(UpstreamResponseIDKey, "") != "" {
		return
	}
	if id := upstreamResponseID(payload); id != "" {
		ctx.SetContext(UpstreamResponseIDKey, id)
	}
}

// upstreamResponseID digs the response id out of one JSON payload, empty when
// it carries none.
//
// Four shapes, probed in the order that resolves each in one lookup: OpenAI
// Chat Completions and every non-streaming body carry it at the top level, the
// Responses API nests it under `response` (its events wrap the response
// object), Anthropic under `message` (only message_start carries one), and
// DashScope names it `request_id`.
//
// The DashScope probe is a hedge, not a path that runs today, and it is worth
// being precise about why so nobody concludes this plugin reads raw provider
// bodies. It does not: Higress sorts wasm plugins by descending priority for
// the request chain (istio pilot, push_context.go `iPriority > jPriority`) and
// Envoy runs encoder filters in reverse of it, so on the response path
// ai-proxy (priority 100) runs *before* token-usage (400). By the time a chunk
// reaches here, qwen.go has already mapped `request_id` onto `id`. What the
// hedge covers is ai-proxy's `protocol: original`, a supported setting that
// passes the provider body through untouched -- GPUStack does not emit it
// today, and if it ever did, the failure would be this id silently vanishing,
// which is the exact outcome the field exists to prevent.
//
// Ordered last, so a converted body's own `id` always wins over a `request_id`
// some upstream added beside it.
//
// Shape-agnostic on purpose. vLLM stamps an `id` on embeddings responses
// (`embd-…`) where OpenAI's own shape has none, and that id is a perfectly good
// thing to locate a record by -- the more so because an embeddings call has no
// other. Gating the top-level probe on the endpoint would throw it away for
// exactly the requests that need it most.
//
// The string-type check is what keeps that from over-reaching: gjson's String()
// renders a number just as happily, so a numeric `id` -- or the `index` some
// rerank upstreams put at the top level -- would come back as a plausible
// looking id that identifies nothing.
//
// Pure so it can be exercised without a proxy-wasm host; the context-touching
// half is captureUpstreamResponseID.
func upstreamResponseID(payload []byte) string {
	for _, path := range []string{"id", "response.id", "message.id", "request_id"} {
		if v := gjson.GetBytes(payload, path); v.Type == gjson.String && v.Str != "" {
			return v.Str
		}
	}
	return ""
...
func processTokenUsage(ctx wrapper.HttpContext, data []byte, maxBufferBytes int) []byte {
	...
	for chunk := range chunks {
		...
		captureUpstreamResponseID(ctx, payload)
...
func reportMetrics(ctx wrapper.HttpContext, config PluginConfig) {
	...
	metrics := ModelUsageMetrics{
		...
		TTFTMillis:          optionalInt64(ctx.GetContext(TimeToFirstTokenDuration)),
		RequestID:           optionalString(ctx.GetStringContext(RequestIDKey, "")),
		UpstreamResponseID:  optionalString(ctx.GetStringContext(UpstreamResponseIDKey, "")),
		UserID:              base.UserID,
...
// optionalString distinguishes "not observed" from "observed as empty" on the
// wire, so the server stores NULL rather than an empty string for a request
// whose id never arrived -- an embeddings call has no upstream response id, and
// a column of empty strings would read as though it did.
...
// optionalInt64 does the same for a context value that may be absent. TTFT
// needs it because zero is a legitimate measurement (a chunk arriving inside
// the same millisecond as the request) and must not read as "never measured",
// which is what the non-streaming path produces.
