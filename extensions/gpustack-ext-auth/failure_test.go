...
func TestFailOpenMintsAMarker(t *testing.T) {
	...
	claims, ok := markerClaimsFor(config, resolvedCaller, "my-org/qwen3-8b", "3192253c1f4a9b7e.gpustack-7")
	if !ok {
	...
	if _, ok := markerClaimsFor(config, identity{State: identityUnresolved}, "m", ""); ok {
